// Study: breakout entries, and whether a volatility squeeze improves them.
//
// Everything tested so far entered on a moving-average cross. A cross is a
// smoother disagreeing with itself — it is late by construction, and on a
// 12-hour bar "late" is a day. Breakout is a different mechanism: price itself
// making a new extreme, with no lag, which is how most documented
// trend-following actually enters.
//
// The second idea is conditioning. A rule can have no edge on average and an
// edge inside one regime, and the regime with the best prior claim is volatility
// contraction — a market that has gone quiet has to go somewhere, and breakouts
// from a squeeze are the one setup with a mechanical story behind them rather
// than a curve-fit. So each rule is run unconditionally and again restricted to
// bars whose ATR sits in the bottom third of its own recent range.
//
// Judged exactly as before: naive t is reported because it is what a backtest
// would tell you, clustered-by-day t is what decides, and the halves have to
// agree. Nineteen configurations are examined, so the honest 5% threshold is
// about |t| > 3.2, not 2.
//
// Uses candles already cached by study-interval.mjs. Run from bot/:
//   node research/study-breakout.mjs
//   INTERVALS=720 node research/study-breakout.mjs
...
function tradeSymbol(candles, { entryLook, exitLook, stopAtr, tpR, exit, trendFilter, squeeze }) {
  ...
  for (let i = Math.max(entryLook, 200) + 1; i < candles.length - 1; i++) {
    ...
    if (up) pos = { side: 'long', entry: next.o, stop: next.o - risk, tp: next.o + risk * tpR, risk, day, at: next.t };
    else if (dn) pos = { side: 'short', entry: next.o, stop: next.o + risk, tp: next.o - risk * tpR, risk, day, at: next.t };
  }
  return out;
}
...
const CONFIGS = [
  { label: 'Donchian 20, 2R target',        entryLook: 20, stopAtr: 1.8, tpR: 2, exit: 'target' },
  { label: 'Donchian 20, 3R target',        entryLook: 20, stopAtr: 1.8, tpR: 3, exit: 'target' },
  { label: 'Donchian 20, trail 10',         entryLook: 20, exitLook: 10, stopAtr: 1.8, exit: 'trail' },
  { label: 'Donchian 40, 2R target',        entryLook: 40, stopAtr: 1.8, tpR: 2, exit: 'target' },
  { label: 'Donchian 40, trail 20',         entryLook: 40, exitLook: 20, stopAtr: 1.8, exit: 'trail' },
  { label: 'Donchian 55, trail 20',         entryLook: 55, exitLook: 20, stopAtr: 2.5, exit: 'trail' },
  { label: 'Donchian 20 + trend, 2R',       entryLook: 20, stopAtr: 1.8, tpR: 2, exit: 'target', trendFilter: true },
  { label: 'Donchian 40 + trend, trail 20', entryLook: 40, exitLook: 20, stopAtr: 1.8, exit: 'trail', trendFilter: true },
  { label: 'Donchian 20 + squeeze, 2R',     entryLook: 20, stopAtr: 1.8, tpR: 2, exit: 'target', squeeze: true },
  { label: 'Donchian 40 + squeeze, trail',  entryLook: 40, exitLook: 20, stopAtr: 1.8, exit: 'trail', squeeze: true },
];
...
if (survivors.length === 0) {
  console.log('SURVIVORS: none. No breakout rule here is worth risking money on.');
} else {
